...
public class ClearUsersTest {
    ...
    public void testClearUsersDeletedUsersFromFile() {
        ...
        Main.main(null);
        JButton button = getButton();
// since clicking the button should end up displaying a JDialog to the user
 //       to report the
// result, we set a timer, which will execute code necessary to complete
    //    the testing.
        createCloseTimer().start();
        button.doClick();
// will continue execution here after the JDialog is closed
// users.csv format
//username, password,timestamp (in format returned by a call like
  //      LocalDateTime.now())
//example
...
// this ignores old dialogs
                        if (dialog.isVisible()) {
                            String s = ((JOptionPane) ((BorderLayout)
                                    dialog.getRootPane()
                                            .getContentPane().getLayout()).getLayoutCompone
                            nt(BorderLayout.CENTER)).getMessage().toString();
